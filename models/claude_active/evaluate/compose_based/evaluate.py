...
# trial_embedding.py has no local copy here, so it resolves via the
# appended models/claude_active path -- same reasoning as trial_graph.py.
from trial_embedding import CriterionEncoder, TrialEncoder, encode_all_trials
# build_naive_baseline_trial_embeds is defined in train.py; safe to import
# since train.py's real work is gated behind `if __name__ == "__main__":`.
...
def build_evaluation_matrices(cfg: Config):
    ...
    # --- Re-encode the HELD-OUT trials fresh -- see models/claude_active/
    # evaluate/evaluate.py for the full explanation of why this replaced
    # loading cfg.TRIAL_EMBED_PATH / TRIAL_EMBED_BASELINE_PATH directly.
    ...
    criterion_encoder_model = CriterionEncoder(cfg.OUT_CHANNELS).to(device)
    criterion_encoder_model.load_state_dict(torch.load(cfg.CRITERION_ENCODER_STATE_PATH, map_location=device))
    criterion_encoder_model.eval()

    trial_encoder_model = TrialEncoder(cfg.OUT_CHANNELS).to(device)
    trial_encoder_model.load_state_dict(torch.load(cfg.TRIAL_ENCODER_STATE_PATH, map_location=device))
    trial_encoder_model.eval()
    ...
    with torch.no_grad():
        ...
        trial_embeds = encode_all_trials(
            ...
            criterion_encoder_model, trial_encoder_model, cfg.OUT_CHANNELS, device,
        )
