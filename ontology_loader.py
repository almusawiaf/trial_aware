...
class DynamicOntologyMapper:
    """
    Extracts medical entities from raw text criteria using dynamic keyword matching
    against loaded MIMIC-III database ontology tables (diagnoses, prescriptions, lab events).
    """

    def __init__(self, icd9_to_icd10_map: Optional[Dict[str, str]] = None):
        self.concept_table: Dict[str, Tuple[str, str]] = {}
        self.search_keys = []
        # Reuse crosswalk to ensure diagnosis codes land in ICD-10 space
    ...
    def load_icd9_and_patient_tables(self, data_dir: str):
        ...
        if os.path.exists(rx_path):
            ...
            for _, row in df_rx.iterrows():
                ...
                if ndc and ndc != "nan" and drug and drug != "nan":
